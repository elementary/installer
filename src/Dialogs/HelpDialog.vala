...
public class HelpDialog : Gtk.Window {
    ...
    construct {
        ...
        var requirements = new Gtk.Label ("Requirements");
        requirements.use_markup = true;

        var dual_booting = new Gtk.Label ("""If you don't have an available partition, you can create one using the "Modify Partitions…" button to open GParted.
...
In your Windows install, open Control Panel and head to "Power Options". Select "Choose what the power buttons do", select "Change settings that are currently unavailable", then disable the "fast startup" setting. Note that Windows updates may occasionally turn this setting back on without asking, so if you are unable to boot into %s, check this setting first.""".printf (os, os, os, os, os));
        ...
        stack.add_titled (requirements, "requirements", _("Requirements"));
