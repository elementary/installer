...
public class Installer.DiskGrid : Gtk.Grid {
    ...
    private async void process_block (UDisks2.Block block) {
        UDisks2.Filesystem filesystem;
        try {
            filesystem = yield Bus.get_proxy (BusType.SYSTEM, "org.freedesktop.UDisks2", ((DBusProxy) block).g_object_path);
        } catch (Error e) {
            critical (e.message);
            return;
        }
    ...
    private static ToggleOSButton? detect_system (GLib.File mount_point, UDisks2.Block block, UDisks2.Filesystem filesystem) {
        ...
        if (linux_identifier.query_exists ()) {
            ...
            return new ToggleOSButton (name, version, icon, block, filesystem);
        }
        ...
        if (windows_identifier.query_exists ()) {
            return new ToggleOSButton ("Windows", null, new GLib.ThemedIcon ("os-windows"), block, filesystem);
        }
        ...
        if (macos_identifier.query_exists ()) {
            ...
            return new ToggleOSButton (product_name, product_version, new GLib.ThemedIcon ("os-unix-mac"), block, filesystem);
        }
    ...
    public class ToggleOSButton : Gtk.ToggleButton {
        ...
        Gtk.Grid grid;
        public ToggleOSButton (string name, string? version, GLib.Icon icon, UDisks2.Block block, UDisks2.Filesystem filesystem) {
            this.block = block;
